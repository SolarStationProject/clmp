    ...
    const irAMiUbicacion = () => {
        ...
        navigator.geolocation.getCurrentPosition(pos => {
            ...
            if (gpsMarker.current) {
                gpsMarker.current.setLatLng(ll);
            } else {
                gpsMarker.current = L.marker(ll, { icon: ICON_GPS }).addTo(map);
            }
            map.setView(ll, 16);
        });
    };
    ...
    return (
        <div style={{ width: '100%', height: '100%', position: 'relative' }}>
            ...
            <div style={{
                ...
                background: 'linear-gradient(to bottom, rgba(0,92,46,0.92) 0%, rgba(0,92,46,0.0) 100%)',
                ...
                <span style={{ fontSize: '12px', color: 'rgba(255,255,255,0.8)', fontWeight: '600' }}>{nombre.split(' ')[0]}</span>
            </div>

            {/* Leyenda de colores */}
            <div style={{
                position: 'absolute', top: '60px', left: '12px', zIndex: 15,
            ...
            <div style={{ position: 'absolute', bottom: '80px', right: '14px', zIndex: 20, display: 'flex', flexDirection: 'column', gap: '10px' }}>
                {/* GPS */}
                <button onClick={irAMiUbicacion} style={{
                    width: '44px', height: '44px', borderRadius: '50%',
                    backgroundColor: '#fff', border: 'none', cursor: 'pointer',
                    boxShadow: '0 2px 8px rgba(0,0,0,0.2)', fontSize: '20px',
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                }}>📡</button>
                {/* Zoom + */}
                <button onClick={() => mapInst.current?.zoomIn()} style={{
                    width: '44px', height: '44px', borderRadius: '50%',
                    backgroundColor: '#fff', border: 'none', cursor: 'pointer',
                    boxShadow: '0 2px 8px rgba(0,0,0,0.2)', fontSize: '22px', fontWeight: '700', color: '#475569',
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                }}>+</button>
                {/* Zoom − */}
                <button onClick={() => mapInst.current?.zoomOut()} style={{
                    width: '44px', height: '44px', borderRadius: '50%',
                    backgroundColor: '#fff', border: 'none', cursor: 'pointer',
                    boxShadow: '0 2px 8px rgba(0,0,0,0.2)', fontSize: '22px', fontWeight: '700', color: '#475569',
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
            ...
            <button
                ...
                style={{
                    ...
                    boxShadow: '0 4px 16px rgba(0,92,46,0.45)',
            ...
            {seleccion && (
                <div style={{
                    position: 'absolute', bottom: '76px', left: '12px', right: '12px', zIndex: 30,
            ...
            <div style={{
                position: 'absolute', top: '60px', right: '12px', zIndex: 15,
                backgroundColor: 'rgba(255,255,255,0.95)', borderRadius: '10px',
