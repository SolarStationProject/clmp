...
function wrap(headerSub: string, bodyHtml: string): string {
    return `<html><body style="background:#f1f5f9;margin:0;padding:0;">
    <div style="${card}">
        <div style="${header}">
        ...
        <div style="${footer}">Este mensaje fue generado automáticamente por CleanMap.<br/>Sistema de Gestión de Microbasurales — UNAB 2026</div>
    </div>
    </body></html>`;
}
...
export async function enviarCodigoVerificacion(email: string, nombre: string, codigo: string): Promise<void> {
    const html = wrap('Verificación de cuenta', `
        ...
        <p style="color:#64748b;font-size:14px;margin:0 0 24px;line-height:1.6">
            Gracias por registrarte en CleanMap. Usa el siguiente código para verificar tu cuenta. Es válido por <strong>1 hora</strong>.
        </p>
        <div style="background:#F0FDF4;border:2px solid #86EFAC;border-radius:14px;padding:24px;text-align:center;margin:0 0 24px">
            <p style="color:#64748b;font-size:13px;margin:0 0 8px;font-weight:600;text-transform:uppercase;letter-spacing:0.08em">Código de verificación</p>
            <p style="color:#005c2e;font-size:42px;font-weight:900;letter-spacing:0.25em;margin:0;font-family:monospace">${codigo}</p>
        </div>
        <p style="color:#94a3b8;font-size:13px;margin:0;line-height:1.6">Si no creaste esta cuenta, ignora este mensaje.</p>
    `);
    await sgMail.send({ to: email, from: FROM, subject: `${codigo} — Tu código de verificación CleanMap`, html });
}
...
export async function enviarConfirmacionLogin(email: string, nombre: string, rol: string): Promise<void> {
    ...
    const html  = wrap('Inicio de sesión exitoso', `
        <p style="color:#374151;font-size:15px;margin:0 0 8px">Hola, <strong>${nombre}</strong></p>
        <p style="color:#64748b;font-size:14px;margin:0 0 24px;line-height:1.6">Se registró un nuevo inicio de sesión en tu cuenta CleanMap.</p>
        ...
        <p style="color:#94a3b8;font-size:13px;margin:0;line-height:1.6">Si no fuiste tú, <a href="${BASE_URL}/demo/hu003" style="color:#005c2e;font-weight:600">recupera tu contraseña</a>.</p>
    `);
    await sgMail.send({ to: email, from: FROM, subject: `Inicio de sesión en CleanMap — ${ahora}`, html });
}
...
export async function enviarEnlaceRecuperacion(email: string, nombre: string, token: string): Promise<void> {
    ...
    const html   = wrap('Recuperación de contraseña', `
        ...
        <p style="color:#64748b;font-size:14px;margin:0 0 24px;line-height:1.6">Recibimos una solicitud para restablecer tu contraseña. El enlace es válido por <strong>1 hora</strong>.</p>
        <div style="text-align:center;margin:0 0 24px">
            <a href="${enlace}" style="display:inline-block;background:linear-gradient(135deg,#005c2e,#0a7c40);color:#fff;font-weight:700;font-size:15px;text-decoration:none;padding:14px 32px;border-radius:12px;">
                Restablecer contraseña →
            </a>
        </div>
        <div style="background:#FFFBEB;border:1px solid #FDE68A;border-radius:10px;padding:14px;margin:0 0 20px">
            <p style="color:#92400E;font-size:13px;margin:0;line-height:1.6">⚠️ Si el botón no funciona:<br/>
            <span style="font-family:monospace;font-size:12px;color:#0284c7;word-break:break-all">${enlace}</span></p>
        </div>
        <p style="color:#94a3b8;font-size:13px;margin:0;line-height:1.6">Si no solicitaste este cambio, ignora este mensaje.</p>
    `);
    await sgMail.send({ to: email, from: FROM, subject: 'Restablece tu contraseña de CleanMap', html });
}

// ── HU006: Notificación de cambio de estado ───────────────────────────────────
export async function enviarNotificacionCambioEstado(
    ...
    const html  = wrap('Estado de tu reporte actualizado', `
        <p style="color:#374151;font-size:15px;margin:0 0 8px">Hola, <strong>${nombre}</strong></p>
        <p style="color:#64748b;font-size:14px;margin:0 0 20px;line-height:1.6">El estado de tu reporte ha sido actualizado.</p>
        ...
        ${comentario ? `<div style="background:#FFFBEB;border:1px solid #FDE68A;border-radius:10px;padding:14px;margin:0 0 16px"><p style="color:#92400E;font-size:13px;font-weight:600;margin:0 0 4px">💬 Comentario:</p><p style="color:#78350F;font-size:14px;margin:0">${comentario}</p></div>` : ''}
        <p style="color:#94a3b8;font-size:12px;margin:0;line-height:1.6">Actualizado el ${ahora}</p>
    `);
    await sgMail.send({ to: email, from: FROM, subject: `[CleanMap] Tu reporte ${codigoReporte} cambió a: ${estadoNuevo}`, html });
}
